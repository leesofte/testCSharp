...
{
	if (!cur_block_ptr_ || !ptr)  return 0;
	...
	while (w > 0)
	{
		...
		p += tow;
		w -= tow;
		if (cur_block_size_ == cur_block_offset_) i_MapNextBlock();
		if (cur_block_ptr_ == 0) break;
	}
	return len - w;
}
...
void CMemoryMapWriter::impl::i_MapNextBlock()
{
	...
	if ( cur_block_ptr_ == 0 ) i_ExpandFileSize();
}
...
void CMemoryMapWriter::impl::i_ExpandFileSize()
{
	...
	file_size_ = ::GetFileSize(handle_file_, 0);
	if (file_size_ == 0xFFFFFFFF) return;
